...
class App(ttk.Frame):
    ...
    def print_FileList(self):
        ...
        try:
            for file in os.listdir(self.get_FilePath()):
                file = os.path.join(self.get_FilePath(), file)
                if os.path.isdir(file):
                    self.var_6.set(self.var_6.get() + "[DIR] " + file + "\n")
                else:
                    self.var_6.set(self.var_6.get() + "[FILE]" + file + str(self.convert_size(os.path.getsize(file))).rjust(120 - len(file), '.') + "\n")
    ...
    def push2(self,event):
        ...
        if selected_file == 'exit' or selected_file == 'quit':
            ...
            self.var_6.set(self.var_6.get() + "\nThere is no directory or file named" + selected_file + "in this directory, please re-input")
