...
for object in objects:
    #print("[" + str(object.RecordNumber) + "]", object.TimeGenerated, object.EventID, object.EventCategory, object.ComputerName, Event_Types.get(object.EventType, "Unknown"), object.SourceName)
    # print(object.Sid)
    evtid = str(winerror.HRESULT_CODE(object.EventID))
    print(evtid)

    if object.Sid is not None:
        domain, user, typ = win32security.LookupAccountSid(None, object.Sid)
        sidDesc = "%s/%s" % (domain, user)
    #print(sidDesc)


    # 각 log별 세부정보를 어떤 방식으로 출력할까? 그냥 보여주기 or 선택한 부분만 보여주기
    msg = win32evtlogutil.SafeFormatMessage(object, "System")
    if msg[0:8] != "<The des":
        print(msg)
    else:
        print()
